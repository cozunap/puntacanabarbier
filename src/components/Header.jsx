  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-navy/80 backdrop-blur-md border-b border-white/10 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 md:px-12 h-20 flex justify-between items-center">
        ...
        {/* Logo */}
        <Link to="/" className="text-gold font-serif text-2xl font-bold tracking-wide hover:opacity-80 transition-opacity">
          Punta Cana Barbershop
        </Link>
        ...
        {/* Book Now Button */}
        <button className="hidden md:block bg-gold text-navy px-6 py-2 rounded-sm text-sm uppercase tracking-widest font-bold hover:bg-gold-light transition-colors premium-hover">
          Book Now
        </button>
      ...
      {/* Mobile Nav */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-navy border-b border-white/10 py-6 px-6 flex flex-col gap-6 shadow-2xl">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
          ...
          <button className="bg-gold text-navy px-6 py-3 rounded-sm text-sm uppercase tracking-widest font-bold mt-4 w-full">
            Book Now
          </button>
        </div>
      )}
